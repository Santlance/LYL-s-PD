...
void TetMeshBSH::compute_hull_approx(unsigned int b, unsigned int n, BoundingSphere& hull) const
{
	...
	if (n == 1)
	{
		// 叶子节点，这里做一个缩放，为了更好地进行缝合和碰撞检测
		Real coef = 1;
		hull.r() *= coef;
	}
}

void TetMeshBSH::init(const Vector3r* vertices, const unsigned int numVertices, const unsigned int* indices, const unsigned int numTets, const unsigned int* face_indices, const unsigned int numFaces, const Real tolerance)
{
	//printf("%d %d \n", numVertices, numTets);
	...
	//{
	//	printf("%d:  %d %d %d %d\n",i, indices[i * 4 + 0], indices[i * 4 + 1], indices[i * 4 + 2], indices[i * 4 + 3]);
	//}
	...
	{
		m_faceSet.insert(Face(face_indices[i * 3 + 0], face_indices[i * 3 + 1], face_indices[i * 3 + 2]));
	}
}
...
void TetMeshBSH::updateCom()
{
	for (unsigned int i = 0; i < m_numTets; i++)
	{
		m_com[i] = 0.25 * (m_vertices[m_indices[4 * i]] + m_vertices[m_indices[4 * i + 1]] + m_vertices[m_indices[4 * i + 2]] + m_vertices[m_indices[4 * i + 3]]);
	}
}

bool TetMeshBSH::isTriangleAFace(d_uint id1, d_uint id2, d_uint id3)
{
	return m_faceSet.count(Face(id1, id2, id3));
}
